...
## Honor PC Helper 与荣耀电脑管家对比

| | Honor PC Helper | 荣耀电脑管家 |
| --- | --- | --- |
| 分发方式 | 单个绿色 exe，开源 | 厂商安装包，闭源 |
| 电池充电阈值 | 支持 | 支持 |
| 键盘背光与熄灭超时 | 支持 | 支持 |
| 性能模式 | 支持 | 支持 |
| 背光计划、触控板边缘手势、快捷键自定义 | 支持 | 不支持 |
| 驱动与固件更新 | 不支持 | 支持 |
| 手机多屏协同、MagicRing、文件互传 | 不支持 | 支持 |

需要驱动更新或手机互联，请保留电脑管家；只需要托盘中的硬件设置，用 Honor PC Helper 即可。电脑管家的功能集因版本和地区而异。
...
本项目就是。Honor PC Helper 以单个绿色 exe 覆盖硬件设置 — 充电阈值、背光、性能模式、触控板行为 — 免安装、无后台服务。它不更新驱动，也不提供手机多屏协同。

### 可以卸载荣耀电脑管家只用这个工具吗？

如果你只是用电脑管家调硬件设置，可以。它写入的数值（充电阈值、背光超时）保存在 BIOS 中并继续生效，Honor PC Helper 通过同一 WMI 接口读取和改写。需要驱动更新或 MagicRing 时随时重新安装电脑管家即可。
