...
键盘本身不执行任何操作：**F7** 只会发出 BIOS 事件，其余工作原本由厂商软件完成。Honor PC Helper 接管了这个键，因此无需安装荣耀电脑管家也能使用。

**F7** 静音或取消静音默认录音设备，即 Windows 声音设置中显示的那个静音开关，并让按键上的指示灯与之保持一致。通过音量合成器等其他途径进行的静音同样会反映到指示灯上；启动时与唤醒后也会重新同步。

指示灯由 BIOS 命令（`04 0B`）控制，而非固件自行点亮 —— 即 Linux 主线内核中的 `MICMUTE_LED_SET`。这正是卸载厂商软件后指示灯会停在最后状态的原因：按键从来没有控制过它。

**Fn+P**（高性能模式）同样受支持，详见上方菜单表格。

背光亮度、振动强度和边缘手势会在唤醒后（包括新式待机，此时使用点亮屏幕的事件而非不可靠的唤醒事件）以及触控板重新连接后重新应用——固件会遗忘这三项设置。唤醒后还会检查充电限制：部分机型在合盖再打开后会丢失充电阈值，应用会把所选区间写回去。
...
**F7 没有反应。** 该键与 Fn+P 依赖同一个 `OemWMIEvent` 订阅，请先在日志中检查订阅是否正常。事件代码因机型而异 —— `0x287` 取自 MagicBook Pro 14。若你的机器发送的是其他代码，按键将保持沉默；请提交 issue 并附上机器报告的代码。
